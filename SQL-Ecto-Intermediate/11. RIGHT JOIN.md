...
`join/5` function is used to do left join in `Ecto`. To know more about `join/5` function, visit my [[08. JOIN]] page. Now let's the `:right` in join.
...
You can see in the above example, all records from `SqlEcto.Hr.Country` are returned. But on `SqlEcto.Hr.Region` only records which has same value in the `region_id` got returned.